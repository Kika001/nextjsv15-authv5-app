...
interface NavBar2Props<T extends MenuItem> {
  domain?: {
    ...
    logo?: React.ReactElement<{ size?: number | string }>;
  };
  navigationMenu?: T[];
  isSticky?: boolean;
  authLinks?: {
...
export function NavBar2<T extends MenuItem>(navBar2Props: NavBar2Props<T>) {
  ...
  const RenderNameAndLogo = ({
    defaultLogo,}: { defaultLogo: React.ReactElement }) => {
    return (
      <Link href={"/"}>
  ...
  function RenderMobileMenu({
    ...
    defaultLogo: React.ReactElement<{ size?: number | string }>;
  }) {
    return (
      <Sheet>
        ...
        <SheetContent className="">
          <SheetHeader>
            <SheetTitle className="flex items-center justify-start gap-2">
              {React.cloneElement(defaultLogo, { size: "25" })}
